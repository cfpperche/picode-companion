...
const motion=window.PiCodeMotion,geometry=window.PiCodeViewGeometry;
...
const groups={
 ...
 display:{label:"Display",anchor:[0,63,105],e:[0,-55,2],info:"4″ HDMI LCD (C) · 720 × 720. Model: 84 × 84 mm glass, 72.53 × 72.53 mm active area; connector and mounting geometry remain provisional.",url:'https://www.waveshare.com/4inch-hdmi-lcd-c.htm'},
...
try{
 ...
 function annotations(W,H){
  ...
  if(dimensionCheck.checked&&mode!=='explode'&&headAngle===0){
   const isolated=$('isolate').checked&&component.value!=='all';
   const measured=isolated?geometry.bounds(parts.filter(p=>opacity(p)>0)):null;
   const low=measured?.low||[-80,0,0],high=measured?.high||[80,184,224];
   const mm=value=>Number(value.toFixed(1)).toLocaleString(window.PiCodeI18n?.locale)+' mm';
   const gap=isolated?Math.max(3,Math.max(...measured.size)*.08):12;
   const dims=[{a:[low[0],low[1]-gap,low[2]],b:[high[0],low[1]-gap,low[2]],s:mm(high[0]-low[0]),o:[0,16]},
    {a:[high[0]+gap,low[1],low[2]],b:[high[0]+gap,low[1],high[2]],s:mm(high[2]-low[2]),o:[24,0]},
    {a:[low[0]-gap,low[1],low[2]],b:[low[0]-gap,high[1],low[2]],s:mm(high[1]-low[1]),o:[-16,0]}];
 ...
 function opacity(p){return geometry.opacity(p,component.value,$('isolate').checked,mode);}
 ...
 function draw(){
  ...
  for(const p of parts.filter(p=>!$('isolate').checked||opacity(p)>0))for(const c of p.corners){
   ...
   if(p.j!=='fixed'&&mode!=='explode'&&!$('isolate').checked){
   ...
   }else{const q=project(motion.transform(c,p.j,headAngle).map((v,i)=>v+off[i]));for(let i=0;i<3;i++){lo[i]=Math.min(lo[i],q[i]);hi[i]=Math.max(hi[i],q[i]);}}
  }
  const center=lo.map((v,i)=>(v+hi[i])/2),margin=dimensionCheck.checked?94:40,s=Math.min(Math.max(1,W-margin)/Math.max(.01,hi[0]-lo[0]),Math.max(1,H-margin)/Math.max(.01,hi[1]-lo[1]))*zoom,scale=[2*s/W,2*s/H],depth=(hi[2]-lo[2])/2+250;
 ...
 root.dataset.error=e.message;root.dataset.static='true';
 loading.textContent=t("Static model preview. Interactive 3D is unavailable in this browser.");
 status.textContent=t("Static model preview. Interactive 3D is unavailable in this browser.");
 const fallback=$('static');fallback.hidden=false;
 const sync=()=>{const current=modeSelect.value;fallback.src='/assets/parts/assembly-'+(current==='inside'?'inside':current==='explode'?'explode':'exterior')+'.svg';};
 sync();modeSelect.addEventListener('change',sync);
 $('head-controls').disabled=true;
 component.addEventListener('change',()=>{status.textContent=t(groups[component.value].info);});
 root.querySelectorAll('[data-finish]').forEach(button=>button.disabled=true);
}
})();
